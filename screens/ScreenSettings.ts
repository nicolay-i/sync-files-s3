...
import {getFilesList} from "../utils/s3_client/s3_client";
import {runDownloadFiles, runUploadFiles} from "../syncFile";
...
export class SampleSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			.addButton((button) => {
				button.setButtonText("Get list of fies");
				button.onClick(() => {
					getFilesList()
				});
		...
		new Setting(containerEl)
			.addButton((button) => {
				button.setButtonText("Run full sync");
